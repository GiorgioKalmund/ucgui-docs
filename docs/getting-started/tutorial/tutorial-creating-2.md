...
and your imagination is the only thing holding you back (_and maybe some bugs from my end or Unity's layout system. I still not quite sure
...
Second, we can observer the fluent pattern in action. It is actually quite trivial to implement, just 
...
When we create a new instance of our derived class it obviously has access both methods. Its new internal 
one and `SetCool`. However, **order of operations matter here**! If you first call the new method and aferwards
...
2. Use the `.Cast<T>` extension to cast your component back down. (Make sure the cast is valid!)
...
This generic instantiator is at the heart of UCGUI. Any class inheriting from 'BaseComponent' can be instantiated via this one method.
